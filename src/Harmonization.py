...
def assign_closest_sector_index_optimized_jit(model_w, model_c, h_array , s_array , v_array , i, j):
    pixels = get_neighboring_pixels_jit(h_array, i, j) 
    omega = get_omega_jit(pixels, h_array , s_array , v_array)
...
@njit
def get_omega_jit(pixels, h_array , s_array , v_array):
    omega = []
    for (x, y) in pixels:
        h = h_array[x][y]
        s = s_array[x][y]
...
def harmony_by_template(h_array, s_array, v_array, model: Modele):
    ...
    for i in range(height):
        for j in range(width):
            ...
            template_c = model.C
            template_w = model.w
            template_edges = np.array([template.bord(i) for i in range(len(template.C))])             
